...
const filterReducer = (state, action) => {
  switch (action.type) {
    ...
    case FILTER_ACTIONS.SYNC_FILTERS:
      return action.payload;
...
const Filters = () => {
  ...
  // Advanced state management with useReducer (for local filter management)
  ...
  // Sync Redux filters with local state
  useEffect(() => {
    if (availableFilters.length > 0) {
      // Update local filters state with data from Redux
      filtersDispatch({ 
        type: "SYNC_FILTERS", 
        payload: availableFilters.map(filter => ({
          ...filter,
          id: filter._id,
          type: filter.key.toLowerCase(),
  ...
  const createFilter = useCallback(async () => {
    ...
    try {
      ...
      // Create filter via Redux
      const result = await dispatch(createFilterAction(filterData));
      ...
      if (result.type.endsWith('fulfilled')) {
        ...
        const newFilter = {
          ...
          key: filterData.key,
          type: filterData.key.toLowerCase(),
      ...
      setValidationErrors({ general: 'Failed to create filter' });
      console.error('Error creating filter:', error);
    }
  }, [filterForm, validateFilterForm, dispatch]);
