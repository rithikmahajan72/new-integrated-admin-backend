...
const AuthFlow = () => {
  ...
  const [loginData, setLoginData] = useState({
    authMethod: "phone",
    email: "",
    phone: "7036567890", // Correct admin phone from database
    password: "",
  });
  ...
  // Handle authentication state changes - removed auto-redirect to allow manual navigation to /auth
  // Users can now visit /auth even if already authenticated
  // Redirect only happens during actual login process
  useEffect(() => {
    console.log('AuthFlow useEffect triggered:', { isAuthenticated, user });
    // Removed automatic redirect - users can stay on auth page even if authenticated
          ...
          {isAuthenticated && user?.isAdmin && (
            <div className="mb-6 p-4 bg-green-50 border border-green-200 rounded-lg">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-green-800 font-medium">Already signed in as {user.name}</p>
                  <p className="text-green-600 text-sm">You can continue to dashboard or login with different account</p>
                </div>
                <button
